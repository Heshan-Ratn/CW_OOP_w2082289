...
public class TicketPool {
    ...
    private final Object vendorLock = new Object();// Separate lock for vendor threads
    private final Object customerLock = new Object();// Separate lock for vendor threads
    ...
    public Object getVendorLock() {
        return vendorLock;
    }

    public Object getCustomerLock() {
        return customerLock;
    }
    ...
    private void loadTickets() {
        try (FileReader reader = new FileReader(TICKETS_FILE)) {
            ...
            System.out.println("Error in ticket data format: " + e.getMessage());
        }
    }
    ...
    public synchronized boolean addTicket(Ticket ticket) {
        if (countAvailableTickets() < maxCapacity) {
            ...
            // Notify customers that a new ticket is available
            synchronized (customerLock) {
                customerLock.notifyAll();  // Notify customers that tickets are available
            }
            return true;
        }
//        else {
...
//        }
    ...
    public synchronized boolean isTicketAvailable(String eventName) {
        return tickets.stream()
                .anyMatch(t -> "Available".equals(t.getTicketStatus()) && eventName.equals(t.getEventName()));
    }
    ...
    public synchronized boolean removeTicket(String eventName, String customerId) {
        ...
        if (ticket != null) {
            ...
            synchronized (vendorLock) {
                vendorLock.notifyAll(); // Notify vendors that space is now available
            }
