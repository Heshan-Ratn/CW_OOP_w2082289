...
public class Customer implements Runnable {
    ...
    private int loadRetrievalRateFromConfig() {
        try (FileReader reader = new FileReader("config.json")) {
            ...
            // Validate ticketPurchaseRate and provide a fallback if missing or invalid
            if (config != null && config.getCustomerRetrievalRate() > 0) {
                return (int) config.getCustomerRetrievalRate();
            } else {
                System.out.println("Invalid or missing ticketReleaseRate in config.json, using default rate.");
    ...
    public void run() {
        ...
        for (int i = 0; i < ticketsToBook; i++) {
            ...
            synchronized (ticketPool.getCustomerLock()) {
                while (!ticketPool.isTicketAvailable(eventName)) {  // While no tickets are available for this event
                    try {
                        if (shouldStop(2, eventName)) return; // Case 2: Stopped while waiting
                        loggerRun.info("No tickets available for event: " + eventName + ", customer " + customerId + " waiting...");
                        ticketPool.getCustomerLock().wait();  // Customer waits for tickets to become available
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt();
                        loggerRun.error("Customer " + customerId + " interrupted while waiting for tickets.");
                        return;
                    }
                }
            }
...
//            }
            ...
            try {
                ...
                if (shouldStop(3,eventName)) return; // Case 3: Stopped after sleep
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
    ...
    private boolean shouldStop(int logCase, String eventName) {
        if (!purchasingTickets.get() || adminStopAllPurchases.get()) {
            if (logCase == 1) {
                ...
                loggerRun.info("Ticket purchasing stopped while waiting for consumer: " + customerId);
            } else if (logCase == 3) {
