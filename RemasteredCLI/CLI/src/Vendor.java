...
public class Vendor implements Runnable {
    ...
    private int loadTicketReleaseRateFromConfig() {
        try (FileReader reader = new FileReader("config.json")) {
            ...
            // Validate ticketReleaseRate and provide a fallback if missing or invalid
            if (config != null && config.getTicketReleaseRate() > 0) {
                return (int) config.getTicketReleaseRate();
            } else {
    ...
    public void run() {
        for (Ticket ticket : ticketBatch) {
            ...
            synchronized (ticketPool.getVendorLock()) {
                while (ticketPool.countAvailableTickets() >= ticketPool.getMaxCapacity()) {
                    try {
                        if (shouldStop(2)) return; // Case 2: Stopped while waiting
                        loggerRun.info("Ticket pool is full. Vendor " + vendorId + " is waiting.");
                        ticketPool.getVendorLock().wait(); // Wait only on the vendorLock
                    } catch (InterruptedException e) {
                        Thread.currentThread().interrupt();
                        loggerRun.error("Vendor thread interrupted while waiting: " + vendorId);
                        return;
                    }
                }
            }
            ...
            try {
                ...
                if (shouldStop(3)) return; // Case 3: Stopped after sleep
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
    ...
    private boolean shouldStop(int logCase) {
        if (!releasingTickets.get() || adminStopAll.get()) {
            if (logCase == 1) {
                ...
                loggerRun.info("Ticket release stopped for vendor: " + vendorId + " while in waiting");
            } else if (logCase == 3) {
